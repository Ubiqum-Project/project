...
# #GRAPH==============================================================
# ggplot()+geom_line(data=TARGET_WORDS.count,aes(x=time,y=ratio))
# ggplot()+geom_line(data=TARGET_WORDS.count,aes(x=time,y=ratio*100+10000))+
#   geom_line(data=Price,aes(x = Date,y=Close))+scale_x_date(breaks = date_breaks("3 days"),labels=date_format("%d-%m"))
# #===================================================================
...
#Target word +word 2
{
  ...
  Used.table<-bigrams_united.count[which(grepl("china",bigrams_united.count$bigram )),]
  Used.table
...
#word1 + target word
{
  ...
  #GRAPH3 Research over time a specific bigram
  Used.table<-bigrams_united.count.end[which(grepl("banned",bigrams_united.count.end$bigram )),]
  Used.table
...
#WORD 1 + BIGRAM
{
  ...
  Used.table<-trigrams_united.count[which(grepl("banned",trigrams_united.count$trigram )),]
  Used.table
...
#BIGRAM + WORD 3
{
  ...
  Used.table<-trigrams_united.count.end[which(grepl("",trigrams_united.count.end$trigram )),]
  Used.table
  ...
  Used.table
  ggplot(Used.table,aes(time, appearance*100*20000+10000)) +
    geom_line(size = 1.3) +
    labs(x = NULL, y = "Word frequency")+
    geom_line(data=Price,aes(x = Date,y=Close))+scale_x_date(breaks = date_breaks("3 days"),labels=date_format("%d-%m"))

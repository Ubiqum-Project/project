...
plot(cleaned.n$cexDate, cleaned.n$cexPrice)


plot(cleaned.n2$date, cleaned.n2$gtrendBitcoinBubble)
...
bla <- ggplot(data = cleaned.n2, aes(x = as.POSIXct(cleaned.n2$cleaned), y = as.numeric(cleaned.n2$gtrendBitcoinBubble*8000+13000)), color = "red") + 
  ...
  geom_point(data = cleaned.n2, aes(x =cleaned.n2$cleaned, y = cleaned.n2$gtrendBitcoinBubble*100+13000), color = "red") +
